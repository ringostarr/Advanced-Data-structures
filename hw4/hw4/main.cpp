...
void dijkstra(struct Graph* graph, int src)
{
	...
	int dist[m];      // dist values used to pick minimum weight edge in cut
...
char** parse(string x,char **mat,int index)
{
	int i = 0;
	for (char& c : x)
	{
...
int main()
{
	...
	for (int i = 0; i < 9; i++)
	{
		for (int j = 0; j < 9; j++)
		{
			if (matrix[i][j] == '1')
			{
				addEdge(graph, matrix[0][j],matrix[i][0], 10);
			}
		}
	}
	printGraph(graph);
